...
// ২. নির্দিষ্ট সার্ভিসের ডাটা নিয়ে আসা (ID অনুযায়ী)
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    try {
...
if (isset($_POST['update_service'])) {
    ...
    $features = $_POST['features']; // Updated from bullet_points
    $price = $_POST['price'];
    ...
    if (empty($title) || empty($price) || empty($delivery_time)) {
        ...
        try {
            // Updated SQL to use 'features' instead of 'bullet_points'
            $sql = "UPDATE services SET title=?, description=?, features=?, price=?, delivery_time=?, status=?, image=? WHERE id=?";
            $stmt = $pdo->prepare($sql);
            if ($stmt->execute([$title, $description, $features, $price, $delivery_time, $status, $image, $id])) {
                $message = "Service Matrix Synchronized Successfully!";
                $message_type = "success";
                
                // আপডেট করা ডাটা পুনরায় নিয়ে আসা
                $stmt = $pdo->prepare("SELECT * FROM services WHERE id = ?");
                $stmt->execute([$id]);
                $service = $stmt->fetch();
            }
...
<head>
    ...
    <title>Update Service | Admin Matrix</title>
...
<body class="text-slate-200 min-h-screen flex items-center justify-center p-6">
    ...
    <div class="glass w-full max-w-2xl p-8 rounded-3xl">
        ...
        <div class="flex justify-between items-center mb-6">
            <a href="manage_services.php" class="text-slate-500 hover:text-white transition">
                <i class="fa-solid fa-arrow-left"></i>
            </a>
            <h2 class="text-xl font-bold text-white italic">Update <span class="text-blue-500">Service</span></h2>
            <div class="text-[10px] font-bold text-slate-600 uppercase tracking-widest">ID: #SRV-0<?= $service['id'] ?></div>
        </div>
        ...
        <form action="" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="title" value="<?= htmlspecialchars($service['title'] ?? '') ?>" required 
                           class="w-full bg-slate-950/50 border border-slate-700 rounded-xl p-2.5 pl-10 text-sm outline-none text-white font-semibold">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="delivery_time" value="<?= $service['delivery_time'] ?? '' ?>" required 
                               class="w-full bg-slate-950/50 border border-slate-700 rounded-xl p-2.5 pl-10 text-sm outline-none text-white">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="features" value="<?= htmlspecialchars($service['features'] ?? '') ?>" 
                           class="w-full bg-slate-950/50 border border-slate-700 rounded-xl p-2.5 pl-10 text-sm outline-none text-white">
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full bg-slate-950/50 border border-slate-700 rounded-xl p-3 text-sm outline-none text-white resize-none leading-relaxed"><?= htmlspecialchars($service['description'] ?? '') ?></textarea>
            </div>

            <div class="flex gap-3">
                <a href="manage_services.php" class="flex-1 text-center bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-xl font-bold text-sm transition active:scale-95">
                    Cancel
                </a>
                <button type="submit" name="update_service" class="flex-[2] bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-bold text-sm transition shadow-lg shadow-blue-900/40 active:scale-95 italic">
                    Sync Changes
                </button>
            </div>
        </form>
